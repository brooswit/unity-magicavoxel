...
public class VoxelDefinition : MonoBehaviour
{
    ...
    void OnValidate()
    {
        ...
        smoothGroupRadius = Mathf.Clamp(Mathf.Round(smoothGroupRadius / 1f) * 1f, 1f, 5f);
    ...
    public Mesh GetMesh(int frame, string paletteName = null)
    {
        ...
        float keyRadius = Mathf.Clamp(Mathf.Round(smoothGroupRadius / 1f) * 1f, 1f, 5f); // Snap to 1 increment, clamp 1..5
